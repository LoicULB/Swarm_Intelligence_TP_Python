...
void Particle::move(){
    ...
    for (int i=0; i < size; i++) {
        velocity[i] *= inertia;
        double congnitive_influence = phi_1* getRandom01() * (pbest.x[i] - current.x[i]);
        double social_influence = phi_2 * getRandom01() * (lbest.x[i] - current.x[i]);
        velocity[i]+=congnitive_influence+social_influence;
        current.x[i]+=velocity[i];
...
void Particle::evaluateSolution() {
	current.eval = optim->evaluate(current.x) ;//returns the cost of the solution, where *v is a solution to the problem
    // handle the update of the personal best
