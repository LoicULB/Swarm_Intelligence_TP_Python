...
void updateGlobalBest(double* new_x, double eval){
    //global_best.x = new_x; //test if change
    for (int i=0;i<n;i++){
...
/* Move the swarm */
/* A swarm move by adding position to velocity */
void moveSwarm() {
    ...
    {
        particle.move();
        if (particle.getPbestEvaluation() < global_best.eval) {
            updateGlobalBest(particle.getCurrentPosition(), particle.getPbestEvaluation());
...
bool terminationCondition() {
    if ((max_iterations != 0 && iterations > max_iterations)
        ||  (max_evaluations != 0 && evaluations > max_evaluations))
        return true;
	return false;
}
...
int main(int argc, char *argv[]) {
    ...
    myfile << "iterations" << "," << "best_solution"<< endl;
	//Iterations loop
	while(!terminationCondition()){
        myfile << iterations << "," << global_best.eval << endl;
        cout << "Best swarm " << global_best.eval << " after " << iterations << " iterations "<< endl;
		moveSwarm();
